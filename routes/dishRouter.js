...
dishRouter.route('/:dishId')
  ...
  .post(authenticate.verifyUser, authenticate.verifyAdmin, (req, res, _next) => {
    res.statusCode = 403;
    ...
    Dishes.findById(req.params.dishId)
      .then((dish) => {
        if (dish != null && dish.comments.id(req.params.commentId) != null) {
          ...
          // Just the author can change the comment
          if (!comment.author._id.equals(req.user._id)) {
            err = new Error('You are not the author of the comment you want to update');
            err.status = 404;
            return next(err);
          }
    ...
    Dishes.findById(req.params.dishId)
      .then((dish) => {
        if (dish != null && dish.comments.id(req.params.commentId) != null) {
          ...
          if (!comment.author._id.equals(req.user._id)) {
            err = new Error('You are not the author of the comment you want to delete');
            err.status = 404;
            return next(err);
          }
